...
namespace DCT.Classes
{
    public class NVM
    {
        ...
        public string RemoveExtraChars(string extraChar, string originalValue)
        {
            while (originalValue.Contains(extraChar + extraChar))
            {
                originalValue = originalValue.Replace(extraChar+extraChar, extraChar);
            }
        ...
        public void OverwriteTagItemsValueTableInDatabase()
        {
            ...
            if (userIDIsValid)
            {
                Helper.CreateCopyOfCurrentDB(TIVManager.GetTagItemValueCOUNT, "CLOSED_THE_APP_AUTO_BACKUP");
                m_dbConnection.Open();
                ...
                foreach (TagItemValue tiv in TIVManager._tivList)
                {
                    ...
                    string tempvalue = (tiv._value == null) ? null : RemoveExtraChars("'", tiv._value);

                    sql = "insert into tagitemvaltable (tag, item, thevalue, userid) " +
                        "values ('" + temptag + "', " +
                        "'" + tempitem + "', " +
                        "'" + DoubleUpQuotes(tempvalue) + "', " +
                        "'" + UserID + "')";                        
